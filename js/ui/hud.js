...
export class HUD {
  ...
  showTrack(info) {
    ...
    // `music.getTrackInfo()` returns the station *id* in `station` and its Korean display name in
    // `stationName` — the banner must never fall back to the raw id while a name is available.
    const station = info.stationKo || info.stationName || info.name || info.station || '라디오';
